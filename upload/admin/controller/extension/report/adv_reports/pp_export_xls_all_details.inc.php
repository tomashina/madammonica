<?php
	...
	if ($results) {
		...
		in_array('all_prod_option', $advpp_settings_all_columns) ? $worksheet->setColumn($j,$j++,13) : '';
		}
		}
		...
		in_array('all_custom_fields', $advpp_settings_all_columns) ? $worksheet->setColumn($j,$j++,16) : '';
		}
		}
		...
		if ($option_names) {
		foreach ($option_names as $option_name) {
		in_array('all_prod_option', $advpp_settings_all_columns) ? $worksheet->writeString($i, $j++, $option_name['name'], $boxFormatText) : '';
		$n++;
		}		 
		}	
		...
		if ($custom_fields_name) {
		foreach ($custom_fields_name as $custom_field_name) {
		in_array('all_custom_fields', $advpp_settings_all_columns) ? $worksheet->writeString($i, $j++, $custom_field_name['name'], $boxFormatText) : '';
		}
		}		
		...
		$j = 0;
			...
			$excelRow = $i+1;			
				...
				in_array('all_custom_fields', $advpp_settings_all_columns) ? $worksheet->write($i, $j++, $custom_field['value'], $textFormat) : '';
				}
				} else {
				foreach ($custom_fields_name as $custom_field_name) {
				in_array('all_custom_fields', $advpp_settings_all_columns) ? $worksheet->write($i, $j++, '', $textFormat) : '';
				}
				}
