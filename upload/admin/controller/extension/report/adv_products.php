...
class ControllerExtensionReportAdvProducts extends Controller { 
	...
	public function index() {
		...
		if (isset($_GET['cron'])) {
			$cron_settings = unserialize($this->config->get('advpp' . $this->user->getId() . 'cron_setting'));
		...
		if (!file_exists(DIR_APPLICATION . 'model/extension/module/adv_settings.php')) {
			$data['module_page'] = $this->response->redirect($this->url->link('extension/module/adv_reports_products', 'user_token=' . $this->session->data['user_token'], true));
		}
		...
		$selected_load_save_reports = unserialize($this->config->get('advpp' . $this->user->getId() . 'sr_load_save_report'));
		...
		$selected_cron_settings = unserialize($this->config->get('advpp' . $this->user->getId() . 'cron_setting'));
		...
		if ($this->config->get('advpp' . $this->user->getId() . '_settings_filters')) {
			...
			$data['advpp_settings_filters'] = array_keys($data['filters']);
		}
		...
		if ($this->config->get('advpp' . $this->user->getId() . '_settings_cm_columns')) {
			...
			$data['advpp_settings_cm_columns'] = array('cm_category','cm_manufacturer','cm_sold_quantity','cm_sold_percent','cm_cart_quantity','cm_wishlist_quantity','cm_total_excl_vat','cm_total_tax','cm_total_incl_vat','cm_app','cm_refunds');
		}
		...
		if ($this->config->get('advpp' . $this->user->getId() . '_settings_mv_columns')) {
			...
			$data['advpp_settings_mv_columns'] = array('mv_id','mv_image','mv_name','mv_model','mv_category','mv_manufacturer','mv_status','mv_stock_quantity','mv_sold_quantity','mv_sold_percent','mv_total_excl_vat','mv_total_tax','mv_total_incl_vat','mv_app','mv_refunds');
		}	
		...
		if ($this->config->get('advpp' . $this->user->getId() . '_settings_pl_columns')) {
			...
			$data['advpp_settings_pl_columns'] = array('pl_prod_order_id','pl_prod_date_added','pl_prod_inv_no','pl_prod_id','pl_prod_sku','pl_prod_model','pl_prod_name','pl_prod_option','pl_prod_currency','pl_prod_price','pl_prod_quantity','pl_prod_total_excl_vat','pl_prod_tax','pl_prod_total_incl_vat');
		}
		...
		$data['initialise'] = '';
		 if (!$query->rows) {
			...
			$settings_data = array(
				'advpp' . $this->user->getId() . '_settings_filters' 		=> array_keys($data['filters']),
				...
				'advpp' . $this->user->getId() . '_settings_cm_columns' 	=> array('cm_category','cm_manufacturer','cm_sold_quantity','cm_sold_percent','cm_cart_quantity','cm_wishlist_quantity','cm_total_excl_vat','cm_total_tax','cm_total_incl_vat','cm_app','cm_refunds'),								
				...
				'advpp' . $this->user->getId() . '_settings_pl_columns' 	=> array('pl_prod_order_id','pl_prod_date_added','pl_prod_inv_no','pl_prod_id','pl_prod_sku','pl_prod_model','pl_prod_name','pl_prod_option','pl_prod_currency','pl_prod_price','pl_prod_quantity','pl_prod_total_excl_vat','pl_prod_tax','pl_prod_total_incl_vat'),
		...
		if (!isset($_GET['cron'])) {
			...
			$cron_settings = unserialize($this->config->get('advpp' . $user_id . 'cron_setting'));
			foreach ($cron_settings as $cron_setting) {
				if (isset($_GET['cron_id']) && ($_GET['cron_id'] == $cron_setting['cron_id'])) {
